...
export default {
  ...
  methods: {
    ...
    addNewCate() {
      ...
      postRequest("/admin/position/add", { name: this.cateName }).then(
        (resp) => {
          if (resp.status == 200) {
            var json = resp.data;
    ...
    handleEdit(index, row) {
      ...
      this.$prompt("请输入新名称", "编辑", {
        ...
        if (value.length == 0 || "               ".indexOf(value) != -1) {
          this.$message({ type: "error", message: "职位种类名不得为空" });
          _this.loading = false;
          return;
        } else if (value.length >= 11) {
          this.$message({
            type: "error",
            message: "职位种类名长度不得超过10个",
          });
          _this.loading = false;
          return;
        }
        _this.loading = true;
        putRequest("/admin/position/update", {
          id: row.id,
          name: value,
        }).then((resp) => {
          var json = resp.data;
    ...
    deleteCate(ids) {
      ...
      this.loading = true;
      //删除
      deleteRequest("/admin/position/delete/" + ids).then(
        (resp) => {
          var json = resp.data;
          _this.$message({
            type: json.status,
