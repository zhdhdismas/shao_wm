<template>
  ...
  <script>
import { postRequest2, putRequest } from "../../utils/api";
  ...
  methods: {
    ...
    deleteSalary(data) {
      this.$confirm(
        ...
        .then(() => {
          deleteRequest("/admin/salary/delete/" + data.id).then((resp) => {
    ...
    nextStep() {
      if (this.activeItemIndex == 10) {
        if (this.salary.id) {
          putRequest("/admin/salary/update", {
            id:this.salary.id,
          ...
          postRequest2("/admin/salary/add", this.salary).then((resp) => {
            if (resp) {
              this.$message({ type: "success", message: resp.data.msg });
              this.initSalaries();
              this.dialogVisible = false;
