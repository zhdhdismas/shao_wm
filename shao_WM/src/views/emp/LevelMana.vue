  ...
  methods: {
    ...
    addNewCate() {
      ...
      if (this.categories.length >= this.maxLimitCategory) {
        this.$message({ type: "error", message: "等级种类已达上限" });
        _this.loading = false;
        return false;
      }
      if (
        ...
        this.$message({ type: "error", message: "等级种类名不得为空" });
        _this.loading = false;
        return false;
      } else if (this.cateName.length >= 11) {
        this.$message({ type: "error", message: "等级种类名长度不得超过10个" });
        _this.loading = false;
        return false;
      }
      postRequest("/admin/level/add", { name: this.cateName }).then((resp) => {
        if (resp.status == 200) {
          var json = resp.data;
    ...
    handleEdit(index, row) {
      ...
      this.$prompt("请输入新名称", "编辑", {
        ...
        inputValue: row.levelName,
        cancelButtonText: "取消",
      }).then(({ value }) => {
        this.loading = true;
        var _this = this;

        if (value.length == 0 || "               ".indexOf(value) != -1) {
          this.$message({ type: "error", message: "等级种类名不得为空" });
          _this.loading = false;
          return;
    ...
    handleDelete(index, row) {
      let _this = this;
      this.$confirm("确认删除 " + row.levelName + " ?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
    ...
    deleteCate(ids) {
      ...
      this.loading = true;
      //删除
      deleteRequest("/admin/level/delete/" + ids).then(
        (resp) => {
          var json = resp.data;
          _this.$message({
            type: json.status,
