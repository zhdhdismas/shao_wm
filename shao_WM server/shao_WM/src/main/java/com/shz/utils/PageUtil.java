...
public class PageUtil {
    ...
    public boolean setPage(String key, String hkey, double score, String value) {
        ...
        try {
            result = redisTemplate.opsForZSet().add("history:"+key + ":page:", hkey, score);
...
//        redisTemplate.expire(key + ":page", 1800000, TimeUnit.MILLISECONDS);
        redisTemplate.expire("history:"+key + ":page:",60 , TimeUnit.HOURS);
        return result;
    }
    ...
    public Set<String> getPage(String key, int offset, int count) {
        ...
        try {
            result = redisTemplate.opsForZSet().rangeByScore("history:"+ key + ":page:", 1, 100000, (offset - 1) * count, count);//1 100000代表score的排序氛围值，即从1-100000的范围
        } catch (Exception e) {
            e.printStackTrace();
        }
        return result;
    ...
    public Integer getSize(String key) {
        Integer num = 0;
        try {
            Long size = redisTemplate.opsForZSet().zCard("history:"+ key + ":page:");
